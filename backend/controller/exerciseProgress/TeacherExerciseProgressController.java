...
@RequestMapping("/skillkids-platform/teacher/exercise-progress")
public class TeacherExerciseProgressController {

    private final ExerciseProgressService service;


    @GetMapping("/student/{studentId}")
    public ResponseEntity<List<ExerciseProgressResponseDto>> findAllByStudent(@PathVariable(name = "studentId") UUID studentId) {
        return ResponseEntity.status(HttpStatus.OK).body(service.findAllForTeacherByStudentId(studentId));
    }
    ...
    public ResponseEntity<ExerciseProgressResponseDto> findByStudentAndExercise(@PathVariable(name = "studentId") UUID studentId, @PathVariable(name = "exerciseId") UUID exerciseId) {
        return ResponseEntity.status(HttpStatus.OK).body(service.findForTeacherByStudentAndExercise(studentId, exerciseId));
    }
}
